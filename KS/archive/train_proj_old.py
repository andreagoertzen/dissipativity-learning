...
class DeepONet(nn.Module):
    def __init__(self,Branch,Trunk):
        ...
        # self.w0 = torch.nn.Parameter(torch.zeros(1,s))
    ...
    def f_project(self,w_in,w_out):
        ...
        w_star = w_out - dVdw * F.relu( torch.einsum('bi,bi->b',A,w_out) + bx)/torch.clamp((dVdw**2).sum(dim=1), min=self.eps_proj)
...
print(y_test.shape)
print(u_test.shape)
axs[0].imshow(y_test.T.detach().cpu().numpy().astype(np.float32),extent=[physical_t[90000]*dt,physical_t[-1]*dt,physical_x[0],physical_x[-1]],aspect='auto')
axs[0].set_title('data')
axs[1].imshow(u_test.T.detach().cpu().numpy().astype(np.float32),extent=[physical_t[90000]*dt,physical_t[-1]*dt,physical_x[0],physical_x[-1]],aspect='auto')
