...
def nan_check(x, name, step, t):
    # prints step and exits if any NaN/Inf is present
    if not torch.isfinite(x).all():
        print(f"[NaN/Inf] detected in {name} at step={step}, t={t:.6f}")
        sys.exit(1)

def parse_args():
    p = argparse.ArgumentParser(description="Kolmogorov flow data generator")
    p.add_argument("--Re", type=float, default=500.0, help="Reynolds number")
    p.add_argument("--dt", type=float, default=5e-4, help="Integration time step")
    p.add_argument("--T", type=float, default=500.0, help="End time")
    p.add_argument("--grid_size", type=int, default=128, help="Number of grid points in x and y (M=N)")
    p.add_argument("--tsave", type=float, default=1.0, help="Time interval between saved frames")
    p.add_argument("--n_traj", type=int, default=50, help="Number of trajectories to generate")
    p.add_argument("--ic_factor", type=float, default=10.0, help="Scaling factor for initial conditions")
    # non-argparse-critical, but handy to keep configurable:
    p.add_argument("--n_ani", type=int, default=5, help="How many trajectories to visualize")
    p.add_argument("--forcing_period_n", type=int, default=4, help="Forcing wavenumber n")
    # p.add_argument("--output_dir", type=str, default="data", help="Output root directory")
    return p.parse_args()
...
def main():
    ...
    Re = args.Re
    dt = args.dt
    n = args.forcing_period_n
    T = args.T
    M = N = args.grid_size
    t_save = args.tsave
    n_traj = args.n_traj # number of trajectories to generate
    n_ani = args.n_ani # how many trajectories to visualize
    ic_factor = args.ic_factor
    ...
    with torch.no_grad():
        for step in tqdm(range(n_steps)):
            ...
            nan_check(omega_hat, "omega_hat", step, step*dt)
            ...
            if step % int(t_save/dt) == 0:
                ...
                nan_check(w, "w (ifft2 of omega_hat)", step, step*dt)
                # print(w.device)
